...
def run_sweep():
  ...
  if current_block_number >= next_sweep:
    ...
    for from_address in config["from_addresses"]:
      ...
      to_sweep_90 = int(0.9 * to_sweep)
      to_sweep_10 = to_sweep - to_sweep_90
      ...
      # Compose the transfer call for 90%
      transfer_extrinsic_90 = substrate.compose_call(
      ...
      # Compose the transfer call for 10%
      transfer_extrinsic_10 = substrate.compose_call(
            call_module   = "Balances",
            call_function = "transfer_keep_alive",
            call_params   = {
                "dest":  config["partner_address"],
                "value": to_sweep_10,
            }
        )
      ...
      if config["proxy_delay"] == 0:
          # Introduce a delay of 15 minutes to allow sufficient time for in-coming transfer to complete
          logging.info("15 min delay before transfers start...")

#          time.sleep(900)
          time.sleep(1)
          logging.info("Delay completed. Initiating transfers...")

          # Send 90% via proxy
          proxy_call(transfer_extrinsic_90, from_address, substrate)
          # Send 10% via proxy
          proxy_call(transfer_extrinsic_10, from_address, substrate)
...
if __name__ == "__main__":
  ...
  if "SWEEP_PARTNER_ADDRESS" in os.environ:
    config["partner_address"] = os.environ["SWEEP_PARTNER_ADDRESS"]
  ...
  schedule.every(5).minutes.do(run_sweep)
